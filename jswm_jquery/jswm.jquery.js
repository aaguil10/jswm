    ...
    */
    ...
    JSWM.prototype.tile = function () {
        ...
        for (var i = 0; i < this.windows.length; i++) {
            //if (this.windows[i]) need to protect untiled windows
    ...
    /*JSWM.prototype.untile = function (jsWindow){
        this.reSizeWindow(jsWindow, 360, 360, put to a random position)//release the window 
        _this.tile(); //retile remaining windows
     ...
     */
    JSWM.prototype.openElement = function (contents, w, h, l, t, options, force, onclose) {

        if (!force && this.isWrapped(contents))
            return; // return if content already wrapped
        var jsWindow = new JSWindow(this, w, h, l, t, options, contents, onclose);
        this.addWindow(jsWindow);
    ...
    JSWindow = function (manager, w, h, l, t, options, contents, on_close) {
        ...
        if (!this.options.noResize) {
            ...
            var dragNW = function (drag, ui) {//this is wrong
                    var relative = {
            ...
            var dragNE = function (drag, ui) {//this is wrong
                    var relative = {
                        left: null,
                        top: $(this.parentNode).offset().top - ui.offset.top
                    },
            ...
            var dragSW = function (drag, ui) {//this is wrong
                    var relative = {
                        left: $(this.parentNode).offset().left - ui.offset.left,
                        top: 0
                    },
    ...
    JSWindow.prototype.close = function () {
        ...
        //console.log(this.manager.windows);
        if(this.manager.windows.length != 0){//windowNum != 0){//make sure windows are still open before trying to close them
            
            this.on_close();//have to call this here to fire the callback of each window f'n on_close. Previous was '_this.on_close' and '_this.container'. Needed curr_active_window to reference its window it was passing, i.e. this.on_close and this.container
            
            this.container.parentNode.removeChild(this.container);

            //alert(JSON.stringify(this.manager.windows));

            //index_ = this.manager.windows.indexOf(this.manager.curr_active_window);
            
            //alert(this.manager.windows.length);
            this.manager.windows.splice(this.manager.curr_active_window,1);
            //alert(this.manager.windows.length);
            
            //this.manager.whichWindow(0); 
        }
